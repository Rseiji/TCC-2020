...
def model_training_normal_cross_validation(tokenized_sentences, attention_masks,
    ...
    optimizer = tm.get_adam_optimizer(model, learning_rate=learning_rate, epsilon=epsilon, weight_decay=weight_decay)
...
def model_given_dataset(sentences_train, sentences_test, labels_train, labels_test, device,
                        batch_size, epochs, learning_rate=2e-5, epsilon=1e-8,
                        bert_model='bert-base-uncased', weight_decay=0):
    model = tm.get_model(bert_model=bert_model, num_labels=2)
    optimizer = tm.get_adam_optimizer(model, learning_rate=learning_rate, epsilon=epsilon, weight_decay=weight_decay)
    ...
    train_dataset, _ = tcv.cross_validation(input_ids_train, attention_masks_train, labels_train, 1)
    test_dataset, _ = tcv.cross_validation(input_ids_test, attention_masks_test, labels_test, 1)
